...
namespace RegIN.Classes
{
    public class User
    {
        ...
        public void GetUserLogin(string Login)
        {
            ...
            if (Connection.OpenConnection(mySqlConnection))
            {
                MySqlDataReader userQuery = Connection.Query($"SELECT * FROM `users` WHERE `Login` = '{Login}'", mySqlConnection);
            ...
            Connection.CloseConnection(mySqlConnection);
        }
        ...
        public void CrateNewPassword()
        {
            if (Login != String.Empty)
            {
                ...
                if (Connection.OpenConnection(mySqlConnection))
                {
                    Connection.Query($"UPDATE `users` SET `Password`='{this.Password}' WHERE `Login` = '{this.Login}'", mySqlConnection);
                }
